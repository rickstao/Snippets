/*	This program was done by myself (Ruikang Tao). My knowledge knowledge in java
 	...
 	and received help from TAs and MSI leaders.
 	I spent about 8 hours working on this program. Thank you for taking your time
 	to grade this assignment.
*/

/**
	*Assignment #4.
	*This program generates a Tic Tac Toe on a 4X4X4 three dimensional board layout.
...
public class TTT3D{
    ...
    public static void main(String[] args) throws Exception{
    	...
    	aiMoved = false; //set ai stationary and wait for the player to make a move

    	int [][][] board = new int[4][4][4]; //declaring 3d array
    ...
    //ai moves every time after the player makes a move
    ...
    public static void printBoard(int[][][] board){
    	int draw = 0;
		//creating the format for the board
    	for(int i=3; i>=0; i--){
    		...
    		for(int j=3; j>=0; j--){
    			...
    			for(int k=0; k<4; k++){
    				...
    				if(board[i][j][k] == 1){
    					...
    					System.out.print("o "); //ai input
    					draw++;
    				}
    				if(board[i][j][k] == 5){
    					System.out.print("x "); //user input
    				}
    			}
    			System.out.println();
    		}
    ...
    //function checks whether games
    public static void checkGame(int [][][] board){
    	for (int i=0; i<76; i++){
    		int sum=0;
    		for (int j=0; j<4;j++){
    		//searches for values in the array
			int a=lines[i][j][0];  
			int b=lines[i][j][1];
			int c=lines[i][j][2];	
			sum+= board[a][b][c];
	...
	//check if rows and columns are filled by summing each slot
	...
	//check if two diagonals are filled by summing each slot
				...
				for (int j=0;j<4 ;j++ ) {
				 	...
				 	for (int k=0;k<4 ;k++,temp--) {
				 		...
				 		if (j!=temp) 
				 			...
				 			m=k;		
					// if statement for valueRow1 and valueRow2
					}
